...
void lara_as_stop(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	if (input & IN_DUCK && lara.water_status != LARA_WADE && item->current_anim_state == AS_STOP && (lara.gun_status == LG_ARMLESS || lara.gun_type == LG_UNARMED || lara.gun_type == LG_PISTOLS || lara.gun_type == LG_AUTOPISTOLS || lara.gun_type == LG_DESERTEAGLE || lara.gun_type == LG_UZIS || lara.gun_type == LG_FLARE))
	{
		item->goal_anim_state = AS_DUCK;
		return;
	}
