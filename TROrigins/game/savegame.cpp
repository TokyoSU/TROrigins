...
static void CreateWeaponStartInfo(long weaponObjNum, long weaponAmmoObjNum, long ammoCount, ushort* got_weapon, ushort* weapon_ammo, long* laraAmmoPtr)
{
	if (Inv_RequestItem(weaponObjNum))
	{
		*got_weapon = TRUE;
		*weapon_ammo = static_cast<ushort>(*laraAmmoPtr);
	}
	else
	{
		*got_weapon = FALSE;
		*weapon_ammo = static_cast<ushort>(Inv_RequestItem(weaponAmmoObjNum) * ammoCount);
	}
}

void CreateStartInfo(long level)
{
	START_INFO* pInfo = &savegame.start[level];
	pInfo->available = TRUE;
	pInfo->pistol_ammo = PISTOL_AMMO;

	if (Inv_RequestItem(PISTOLS_ITEM))
		pInfo->got_pistols = TRUE;
	else
	...
	CreateWeaponStartInfo(GRENADE_GUN_ITEM, GRENADE_AMMO_ITEM, GRENADE_AMMO, &pInfo->got_grenade, &pInfo->grenade_ammo, &lara.grenade.ammo);
	CreateWeaponStartInfo(ROCKET_GUN_ITEM, ROCKET_AMMO_ITEM, ROCKET_AMMO, &pInfo->got_rocket, &pInfo->rocket_ammo, &lara.rocket.ammo);
	CreateWeaponStartInfo(HARPOON_ITEM, HARPOON_AMMO_ITEM, HARPOON_AMMO, &pInfo->got_harpoon, &pInfo->harpoon_ammo, &lara.harpoon.ammo);
...
void CreateSaveGameInfo()
{
	...
	ResetSG();
	ZeroMemory(savegame.buffer, sizeof(savegame.buffer));
